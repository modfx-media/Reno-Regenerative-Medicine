...
function uniqueSlug(title: string, contentId: string, taken: Set<string>, localSlugs: Set<string>): string | null {
  const base = slugFromTitle(title)
  if (localSlugs.has(base)) return null
...
export async function getLiveRankedBlogPosts(
  ...
  if (projectId && projectId !== id) {
    console.error(`[ranked] refusing other project ${projectId}`)
    return []
  }
  ...
  try {
    ...
    if (shouldRefuseForeignCalendar(liveBlogs)) {
      console.error('[ranked] calendar belongs to another market; refusing import')
      return []
    }
    const candidates = liveBlogs.filter((item) =>
      isThisSiteArticle(item.title, item.description, item.source_url || item.document_url),
    )
    ...
    for (const row of resolved) {
      ...
      const slug = uniqueSlug(source.title, source.id, taken, localSlugs)
      if (!slug) continue
...
export async function getRankedOnlyBlogSlugs(): Promise<string[]> {
