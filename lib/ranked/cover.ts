...
import { COMMITTED_COVERS, COMMITTED_COVER_SLUGS, coverPrompt } from './config'
...
function committedCoverUrl(slug?: string): string | null {
  if (!slug) return null
  if (COMMITTED_COVERS[slug]) return COMMITTED_COVERS[slug]
...
function picsumUrl(slug: string): string {
  return `https://picsum.photos/seed/${encodeURIComponent(`reno-regen-${slug}`)}/1200/630`
}
